(function () {
  ...
  function freshBuiltinForStoredProfile(raw) {
    const defaultProfile = builtinProfiles.get(PROFILE_BUNDLE.defaultProfileId) || PROFILE_BUNDLE.profiles?.[0];
    if (!defaultProfile || raw?.customized !== false) return null;

    const baseProfile = builtinProfiles.get(raw?.baseProfileId);
    if (baseProfile) return baseProfile;

    if (String(raw?.name || '') === defaultProfile.name) return defaultProfile;
    return null;
  }
  ...
  function initializeRecommendationProfile() {
    ...
    if (stored) {
      try {
        ...
        const freshBuiltin = freshBuiltinForStoredProfile(raw);
        if (freshBuiltin) {
          applyProfileSnapshot(snapshotFromBuiltin(freshBuiltin), { persist: true });
        } else {
          applyProfileSnapshot(raw, { persist: false });
        }
        return;
      } catch (error) {
